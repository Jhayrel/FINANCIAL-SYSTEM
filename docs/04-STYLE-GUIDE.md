...
| Filters | One row of dropdown chips (`FilterChip`), not rows of pills running off the edge (owner, 2026-09-26: "make it cleaner"). Each opens the phone's own picker; a chip set away from its default is marked in `--brand-100` with a `--brand-600` border. Under them one quiet line gives the count, and Clear filters when any is set. The Database and Activity. |
| Sideways strips | A strip wider than the screen (Settings' sections, the months, quick picks) fades at an edge while there is more to scroll that way and is sharp at its ends, driven by its own scroll position. A word cut at the edge otherwise read as a broken layout. |
| AI cards | The buttons are reversed on a phone, each an equal share of the width, so the main one (Add to ledger) is on the right and Discard on the left. |
| The bell's list | Dims the page behind it, like a sheet. |
...
12. **A wallet is one field that shows its balance** (owner, 2026-09-16, replacing a box per wallet: "what if I have 10+ banks, this area will be congested"). The closed field reads "Cash ₱19,497.00". Its list shows every wallet once with its balance on the right (below zero in `--over`), the usual one tagged "Usual", Wallets (spending and reserve) and Savings under their own headings, and a search box once there are more than eight. While nothing is chosen, "Use Cash" beside the label picks the usual one in one tap. A transfer's destination leaves out the wallet the money leaves. A debt is the same kind of field, showing what each owes, and is picked by itself when there is only one. Its effect is a slim row of choices ("Borrow more", "Pay it down", "Interest or fee", "Forgiven"), and only the one wallet that effect moves money through appears, after it is picked. **The date is Today, Yesterday, or the date field.** The form header says where a save goes: "Connected to the database", "Saving 2 changes", "Offline: 1 change kept here", or "Saving on this device only" when not signed in.
...
**Notices** (toasts, a newer version, a change that did not save, the connection): one stack, fixed **under the top bar at the right** on a desktop and across the width under the bar on a phone. The bottom is where the work is: the Save bar, the chat's composer, the AI button and the phone's tab bar, and a notice at the bottom covered one of them whichever corner it took. The top right is beside the bell, where the owner asked notifications to live. A notice is the inline alert's parts on a card: `--surface`, 1px `--hairline`, `radius-md`, `--shadow-overlay`, a close button (44px to hit) in the corner. A one-line toast keeps its button on the same line. Toasts go after 6s, and the clock stops while the notice is pointed at or focused. Carries at most two actions. Never more than three toasts at once, newest at the bottom. Every notice can be closed, a saving problem included; closing takes it out of the corner and it stays listed at the top of the bell's list until it is put right. While the bell's list is open the stack is hidden, so the two never overlap (the notice sat over the list's first line, owner, 2026-09-26). A saving problem names what was refused (an entry, a setting, a budget, a read, the activity trail) and says whether the entries are affected; "A change did not save" with no subject is not allowed.
